...
img = imresize(img,[dim_img,dim_img]);
 
sh = randperm(dim_img^2/64);
img = shuffle(img,sh);
features = extract_features(img,Q);

prag = compute_thr(dim_img, sh, features, Q, 'lena_50.jpg'); %% !!!!aici trebuie vazuta o denumire automata cu strcat ca sa ia referinta ex: 'lena.jpg_50.jpg'

%% de aici pana la sfarsit
%%
%% sectiune prelucrare date inainte de trimitere
...
%% sfarsit sectiune prelucrare date
%%
%% scriere + trimitere
write_in_header(dim_prag, prag_array, sh_zero, features_abs, sign_of_features , fileName);

%% citire si verificare
